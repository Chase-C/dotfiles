...
5. Present the plan to the user as a normal response (not the `ask_user` tool) before dispatching: list the waves in order with task filenames and a one-line summary of each task's `What to build`. Ask for confirmation (or any sequencing overrides). If they approve, continue to Stage 2; if they request changes, apply them and re-confirm.

### Stage 2 — Dispatch the work

Process waves in order. For each wave:

1. **Dispatch.** Invoke a `tdd-implementer` subagent for every task in the wave **in a single turn** so the invocations run in parallel. Each invocation passes the path to that one task file. Never bundle multiple tasks into one subagent.
...
4. **Decide.**
   ...
   - Otherwise, advance to the next wave.
